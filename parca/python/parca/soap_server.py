...
class ParcaJob(Base):
    ...
    def __init__(self, ip, title, seq1, seq2, tag="", limit=40, gep=1.0, matr="blosum62"):
        self.ip = ip
        self.sequence1 = seq1
        self.sequence2 = seq2
        self.title = title
...
def start_new_job(ip, title, seq1, seq2, tag="", limit=40, gep=1.0, matr="blosum62"):
    session = Session()
    job = ParcaJob(ip, title, seq1, seq2, tag, limit, gep, matr)
    session.add(job)
    session.commit()
    return job.id
