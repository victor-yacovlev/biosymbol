...
bool Matrix::set(int row, int col, const std::deque<A> & value)
{
    bool status = true;
    A stop;
    stop.l = STOP_FLAG;
...
bool Matrix::writeRowToStorage()
{
    if ( !m_dropRowsMode ) {
        if (m_storeRowsUsed==m_storedRowsCount) {
           return writeStorageToFile();
        }
        else {
           m_storeRowsUsed++;
       }
...
bool Matrix::writeStorageToFile()
{
