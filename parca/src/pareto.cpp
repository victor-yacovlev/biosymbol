...
void Aligner::directStage(const std::wstring &SY, const std::wstring &SX)
{
    ...
    writeSuccess = writeSuccess && M ->set(0,0, M_00);
    writeSuccess = writeSuccess && MX->set(0,0, MX_00);
    writeSuccess = writeSuccess && MY->set(0,0, MY_00);
    ...
    for (int x=1; x<=SX.length(); x++) {
        ...
        writeSuccess = writeSuccess && M ->set(0,x,M_0x);
        writeSuccess = writeSuccess && MX->set(0,x,MX_0x);
        writeSuccess = writeSuccess && MY->set(0,x,MY_0x);
    }
    ...
    for (int y=1; y<=SY.length(); y++) {
        ...
        writeSuccess = writeSuccess && M ->set(y,0,M_y0);
        writeSuccess = writeSuccess && MX->set(y,0,MX_y0);
        writeSuccess = writeSuccess && MY->set(y,0,MY_y0);
    }

    if (!writeSuccess)
        m_lastError = "Can't write to swap file";
    ...
    for (int y=1; y<=SY.length(); y++) {
        for (int x=1; x<=SX.length(); x++) {
            ...
            paretizeSet(T);
            writeSuccess = writeSuccess && M ->set(y,x,T);
            ...
            T.insert(T.end(), TD.begin(), TD.end());
            paretizeSet(T);
            writeSuccess = writeSuccess && MX->set(y,x,T);
            ...
            T.insert(T.end(), TD.begin(), TD.end());
            paretizeSet(T);
            writeSuccess = writeSuccess && MY->set(y,x,T);
